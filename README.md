...
- タイムラインの先頭にアクセス総数、総成功回数、総失敗回数、sudo総実行回数、su総実行回数を表示（`ja` なしの場合は英語ラベル）
- sudo/suはauth.log上に接続元IPが直接残らないため、指定IPからログイン成功したユーザーの操作として推定できる範囲で表示
#### 国・地域警告
- 接続元IPに紐づく国・地域情報がログに記録されている場合、そのIPと国・地域を警告表示
#### ブルートフォース警告
- 同一IPから同一ユーザーへ短時間に連続失敗が発生した場合、IP、対象ユーザー、検知期間、失敗回数を警告表示
- 検知条件は `1分以内に10回`、`5分以内に30回`、`10分以内に50回`
#### パスワードスプレー警告
- 同一IPから短時間に多数ユーザーへのログイン試行が発生した場合、IP、検知期間、失敗回数、ユニークユーザー数、ユーザーあたり平均失敗回数を警告表示
...
`ja` を付けない場合は英語ラベル、`ja` を付けた場合は日本語ラベルで表示する。
出力例:
...
複数条件に該当する場合は、そのIP・ユーザーで最も広い条件に該当した代表期間と件数を表示する。
